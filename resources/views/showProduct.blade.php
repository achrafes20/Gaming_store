        ...
        <div class="row">
            ...
            <div class="col-lg-6">
                    ...
                    <div class="carousel-inner">
                        <div class="carousel-item active">
                            <div class="product-image mb-4 neon-border-pink p-2" style="height: 400px;">
                                <img src="{{ asset($product->imagepath) }}" class="d-block w-100" alt="{{ $product->name }}">
                            </div>
                        </div>
                        @foreach ($product->ProductPhotos as $index => $item)
                        <div class="carousel-item">
                            <div class="product-image mb-4 neon-border-pink p-2" style="height: 400px;">
                                <img src="{{ asset($item->imagepath) }}" class="d-block w-100" alt="{{ $product->name }} variation {{ $index + 1 }}">
                            </div>
                        </div>
                        @endforeach
                    </div>
                ...
                <div class="thumb-container">
                    <div class="thumb">
                        <img src="{{ asset($product->imagepath) }}" class="thumb-img active" alt="{{ $product->name }}" data-bs-target="#productCarousel" data-bs-slide-to="0">
                    </div>
                    @foreach ($product->ProductPhotos as $index => $item)
                    <div class="thumb">
                        <img src="{{ asset($item->imagepath) }}" class="thumb-img" alt="{{ $product->name }} variation {{ $index + 1 }}" data-bs-target="#productCarousel" data-bs-slide-to="{{ $index + 1 }}">
                    </div>
                    @endforeach
                </div>
                @else
        ...
        <div class="review-section">
            ...
            @if($canReview)
                <div class="cyber-review-form">
                    ...
                    <form action="{{ route('review_products.store', $product->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        ...
                        <button type="submit" class="btn btn-cyber"><i class="fas fa-paper-plane me-2"></i>SUBMIT REVIEW</button>
                    </form>
                </div>
            @elseif(auth()->check())
       ...
       <div class="row" id="productsGrid">
                @foreach ($relatedProducts as $item)
                    <div class="col-lg-3 col-md-4 col-sm-6 mb-4 _{{ $item->category_id }}">
                        <div class="product-card p-3" style="height: 100%;">
