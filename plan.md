...
**Statut : ✅ Terminé — pipeline complet écrit et validé localement (lint/tests/audits réellement exécutés), plus 96 tests métier écrits pour les 4 services HTTP et 3 bugs réels supplémentaires trouvés et corrigés en les écrivant.**
...
**Vérification** : push sur une branche modifiant un seul service → seul le job de ce service se déclenche dans Actions (logique confirmée par la structure `fromJSON`, non testée sur un vrai push GitHub faute d'accès à ce repo distant dans cette session) ; un commit local avec du code mal formaté est rejeté par le hook avant même d'atteindre GitHub (**testé réellement, voir ci-dessus**) ; `php artisan test` exécuté avec succès sur les 4 services testés (96/96) ; `pint --test`, `composer audit`, `npm audit` exécutés localement avec succès sur les 5 services.
